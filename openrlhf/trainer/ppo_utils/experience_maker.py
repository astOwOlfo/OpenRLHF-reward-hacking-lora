...
class RemoteExperienceMaker(NaiveExperienceMaker):
    ...
    def _generate_vllm(self, all_examples: List[dict], **kwargs) -> List[Samples]:
        ...
        for i, llm in enumerate(llms):
            ...
            if prompt_token_ids:
                if vars(self.strategy.args).get("env_file", None) is not None:
                ...
                if full_data[0] is not None:
                    ...
                    for i, (conversation, reward) in enumerate(outputs):
                        ...
                        for turn in conversation.tokens_by_turn:
                            ...
                            current_action_mask.extend([0.0] * (len(prompt_tokens) - 1))  # User prompt
                            current_action_mask.extend(([1.0] * len(response_tokens)) + [0.0])  # Assistant response
